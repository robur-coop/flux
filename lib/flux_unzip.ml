...
let find_eocd ic =
  ...
  let rec go idx =
    ...
    else if String.get_int32_le tail idx = 0x06054b50l
...
let find_central_directory ic =
  ...
  if entries <> 0xffff && cd_size <> 0xffffffffl && cd_off <> 0xffffffffl
  then Ok (Int64.of_int entries, u32_to_i64 cd_off)
  else if Int64.compare eocd_pos 20L < 0
  then error_msgf "Flux_unzip: missing zip64 locator"
  else
    let* locator = really_read ic ~pos:(Int64.sub eocd_pos 20L) 20 in
    if String.get_int32_le locator 0 <> 0x07064b50l
    then error_msgf "Flux_unzip: missing zip64 locator"
    else
      let eocd64_pos = String.get_int64_le locator 8 in
      let* eocd64 = really_read ic ~pos:eocd64_pos 56 in
      if String.get_int32_le eocd64 0 <> 0x06064b50l
      then error_msgf "Flux_unzip: malformed zip64 end of central directory"
      else
...
let parse_extra ~usz ~csz ~offset extra =
  let rec go idx usz csz offset =
    if idx + 4 > String.length extra then (usz, csz, offset)
      ...
      end else go (idx + 4 + sz) usz csz offset
    end in
    go 0 usz csz offset

let parse_entry ic pos =
  let* cdfh = really_read ic ~pos 46 in
  if String.get_int32_le cdfh 0 <> 0x02014b50l
  then error_msgf "Flux_unzip: malformed central directory"
  else if String.get_uint16_le cdfh 8 land 0x1 <> 0
  then error_msgf "Flux_unzip: encrypted members are not supported"
  else
    let* meth = match String.get_uint16_le cdfh 10 with
      | 0 -> Ok `Stored
      | 8 -> Ok `Deflated
      | meth -> error_msgf "Flux_unzip: unsupported compression method %d" meth in
    ...
    let* extra = really_read ic ~pos:(Int64.add pos (Int64.of_int (46 + name_len))) extra_len in
    let usz, csz, offset = parse_extra ~usz ~csz ~offset extra in
    let mtime = to_ptime ~time ~date in
    let next = Int64.add pos (Int64.of_int (46 + name_len + extra_len + comment_len)) in
  ...
  let process ic =
    ...
    let rec go acc pos = function
      ...
      | n ->
        ...
        go (entry :: acc) next (Int64.pred n) in
    let* entries = go [] cd_off entries in
    Ok { filename; entries } in
  match In_channel.open_bin filename with
  | ic -> Fun.protect ~finally:(fun () -> In_channel.close ic) @@ fun () -> process ic
...
let raw ~filename ~pos ~len : string Flux.source =
  let init () =
    ...
    | Ok lfh ->
      ...
      In_channel.seek ic data;
      (ic, len)
  and pull (ic, rem) =
    if Int64.compare rem 0L <= 0 then None
    else
      let len = if Int64.compare rem 0x7ffL < 0 then Int64.to_int rem else 0x7ff in
      match In_channel.really_input_string ic len with
...
let stream t entry =
  ...
  let via = match entry.meth with
    | `Stored -> Flux.Flow.identity
    | `Deflated -> Flux.Flow.(bstr ~len:0x7ff << Flux_de.inflate) in
